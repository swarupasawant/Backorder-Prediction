...
class DataIngestion:
    ...
    def download_backorder_data(self,) -> str:
        try:
            ...
            backorder_tgz_file = os.path.basename(download_url)
            backorder_tgz_file_name = backorder_tgz_file[:-9]

            tgz_file_path = os.path.join(tgz_download_dir,backorder_tgz_file_name)
    ...
    def extract_file(self,tgz_file_path:str):
        try:
            ...
            with tarfile.open(tgz_file_path) as backorder_file_obj:
                backorder_file_obj.extractall(path=raw_data_dir)
    ...
    def split_data_as_train_test(self)-> DataIngestionArtifact:
        try:
            ...
            backorder_dataframe = pd.read_csv(backorder_file_path)
            ...
            for train_index,test_index in split.split(backorder_dataframe,backorder_dataframe["went_on_backorder"]):
                start_train_set = backorder_dataframe.loc[train_index].drop(["went_on_backorder"],axis=1)
                start_test_set = backorder_dataframe.loc[test_index].drop(["went_on_backorder"],axis=1)
